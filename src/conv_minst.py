...
def set_structure(out_channel_1, out_channel_2, k):
    Kernel1 = Tensor(np.random.randn(out_channel_1, 1, 3, 3).astype(np.float32) / np.sqrt(3*3))
    
    Kernel2 = Tensor(np.random.randn(out_channel_2, out_channel_1, 3, 3).astype(np.float32) / np.sqrt(out_channel_1*3*3))
    
    W1 = Tensor(np.random.randn(out_channel_2*7*7, k).astype(np.float32) / np.sqrt(k))
    return [Kernel1, Kernel2, W1]
...
def forward(X, weights):
    ...
    # 第一层卷积
    X1 = convolution(X, Kernel1)
    X2 = maxpooling(X1)
    X3 = relu(X2)

    # 第二层卷积
    X4 = convolution(X3, Kernel2)
    X5 = maxpooling(X4)
    X6 = relu(X5)
    

    # 线性变换
    X7 = reshape(X6, (X6.shape[0], -1))
    logits = fullyconnected(X7, W1)
...
def train_nn(X_tr, y_tr, X_te, y_te,  out_channel_1=4, out_channel_2=4,
             epochs=10, lr=0.5, batch=100,  using_adam=False):
    """ 
    训练过程
    """
    n, k = X_tr.shape[1], y_tr.max() + 1
    np.random.seed(9)
    weights = set_structure(out_channel_1, out_channel_2, k)
    if(using_adam):
        pass
    else:
...
if __name__ == "__main__":
    ...
    ## using SGD optimizer 
    train_nn(X_tr, y_tr, X_te, y_te, epochs= 10, lr = 0.05, batch=100)